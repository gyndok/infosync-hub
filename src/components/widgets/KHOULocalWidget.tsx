...
interface KHOULocalWidgetProps {
  onRemove?: () => void;
}

export const KHOULocalWidget = ({ onRemove }: KHOULocalWidgetProps = {}) => {
    ...
    const fetchNews = async () => {
      try {
        ...
        const { data, error: fetchError } = await supabase.functions.invoke('rss-fetcher', {
          body: { sources: ['khou_local'], limit: 8 }
        });
        ...
        if (data?.success && data.items?.length) {
          ...
          setError('No local news available');
        }
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Failed to fetch local news');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <WidgetContainer title="Houston Local News" className="h-full" onRemove={onRemove}>
        <div className="flex items-center justify-center h-full">
          <div className="animate-pulse text-muted-foreground">Loading local news...</div>
        </div>
      </WidgetContainer>
    );
  }
