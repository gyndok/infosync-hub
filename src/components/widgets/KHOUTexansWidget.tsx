...
interface KHOUTexansWidgetProps {
  onRemove?: () => void;
}

export const KHOUTexansWidget = ({ onRemove }: KHOUTexansWidgetProps = {}) => {
    ...
    const fetchNews = async () => {
      try {
        ...
        const { data, error: fetchError } = await supabase.functions.invoke('rss-fetcher', {
          body: { sources: ['khou_texans'], limit: 8 }
        });
        ...
        if (data?.success && data.items?.length) {
          ...
          setError('No Texans news available');
        }
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Failed to fetch Texans news');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <WidgetContainer title="Houston Texans" className="h-full" onRemove={onRemove}>
        <div className="flex items-center justify-center h-full">
          <div className="animate-pulse text-muted-foreground">Loading Texans news...</div>
        </div>
      </WidgetContainer>
    );
  }
