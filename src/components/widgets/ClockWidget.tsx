...
const getTimezoneLongitude = (timezone: string): number => {
  ...
  try {
    ...
    const offsetHours =
      (localTime.getTime() - targetTime.getTime()) / (1000 * 60 * 60);
...
export const ClockWidget = ({ onRemove }: ClockWidgetProps = {}) => {
  ...
  const sortTimezonesGeographically = (timezones: TimeZoneConfig[]) => {
    ...
    const westTimezones = otherTimezones
      ...
      .sort(
        (a, b) =>
          getTimezoneLongitude(b.timezone) - getTimezoneLongitude(a.timezone),
      ); // Descending for west
    ...
    const remaining = [...westTimezones, ...eastTimezones].filter(
      (tz) => !result.includes(tz),
    );
  ...
  const removeTimezone = async (timezoneId: string) => {
    if (selectedTimezones.length > 1) {
      ...
      const newTimezones = selectedTimezones.filter(
        (tz) => tz.id !== timezoneId,
      );
      const sortedTimezones = sortTimezonesGeographically(newTimezones);
      setSelectedTimezones(sortedTimezones);
      ...
      if (
        timezone &&
        (timezone as any).isCustom &&
        (timezone as any).customId
      ) {
          ...
          <div className="flex gap-8 flex-wrap justify-center">
            {sortTimezonesGeographically(selectedTimezones).map((tz) => (
              <div key={tz.id} className="group relative">
                <AnalogClock timezone={tz.timezone} city={tz.city} time={now} />
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={setNewTimezoneInput}
                      >
                    ...
                    <Button
                      ...
                      className="w-full"
                      disabled={
                        !newCityInput.trim() || !newTimezoneInput.trim()
                      }
                    >
