    ...
    const fetchBreakingNews = async () => {
      try {
        ...
        try {
          ...
          const { data: rssData } = await supabase.functions.invoke(
            "rss-fetcher",
            {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to fetch breaking news",
        );
      } finally {
        setLoading(false);
      }
  ...
  if (error) {
    return (
      <Alert className="mb-4 border-destructive">
        ...
        <AlertDescription>Breaking news unavailable: {error}</AlertDescription>
      </Alert>
    );
  }
    ...
    return (
      <div className="bg-red-600 text-white py-2 overflow-hidden">
        <div className="flex items-center">
          <Badge
            variant="secondary"
            className="bg-white text-red-600 font-bold mr-4 ml-4"
          >
            BREAKING
          </Badge>
  ...
  return (
    <div className="bg-red-600 text-white py-2 overflow-hidden relative">
      <div className="flex items-center">
        <Badge
          variant="secondary"
          className="bg-white text-red-600 font-bold mr-4 ml-4 shrink-0 relative z-10"
        >
          BREAKING
        </Badge>
