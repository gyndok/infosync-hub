...
const ApiHealthDashboard = () => {
  const { getHealthStatus, getSecretsStatus, testApiKey, loading } =
    useApiProxy();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Monitor API services, rate limits, and configurations
          </p>
        </div>
        <Button
          onClick={fetchData}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {healthData?.summary?.length
                ? Math.round(
                    healthData.summary.reduce(
                      (sum, s) => sum + s.avg_response_time_ms,
                      0,
                    ) / healthData.summary.length,
                  )
                : 0}
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {secretsData?.secrets_status?.filter((s) => s.has_secret)
                .length || 0}
            </div>
            <p className="text-xs text-muted-foreground">
              Of{" "}
              {secretsData?.secrets_status?.filter((s) => s.is_required)
                .length || 0}{" "}
              required
            </p>
          </CardContent>
              ...
              <div key={service.service_name} className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge
                      variant={service.is_enabled ? "default" : "secondary"}
                    >
                    ...
                    {secretInfo && (
                      <Badge
                        variant={
                          secretInfo.has_secret ? "default" : "destructive"
                        }
                      >
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <div className="text-center">
                      <div
                        className={`font-semibold ${getUptimeColor(service.uptime_percentage)}`}
                      >
                    ...
                    <div className="text-center">
                      <div className="font-semibold">
                        {service.total_requests_24h}
                      </div>
                      <div className="text-muted-foreground">24h Requests</div>
                    </div>
                    {secretInfo &&
                      !secretInfo.has_secret &&
                      secretInfo.is_required && (
                        <Button
                          variant="outline"
                          size="sm"
      ...
      {secretsData && (
        <Card>
          ...
          <CardContent className="space-y-4">
            {secretsData.secrets_status.some(
              (s) => !s.has_secret && s.is_required,
            ) && (
              <Alert>
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  Some required API keys are missing. Add them to enable full
                  functionality.
                </AlertDescription>
              </Alert>
            )}
              ...
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant={secret.has_secret ? "default" : "secondary"}
                    >
