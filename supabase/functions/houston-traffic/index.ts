    ...
    const mockAlerts: MetroAlert[] = [
      {
        ...
        description:
          "Red Line experiencing 5-10 minute delays due to signal issues near Downtown Transit Center.",
...
async function fetchGoogleTrafficIncidents(): Promise<TrafficIncident[]> {
  if (!googleMapsApiKey) {
    console.log(
      "Google Maps API key not configured, skipping traffic incidents",
    );
    return [];
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Google Places API error: ${data.error_message || response.statusText}`,
      );
    }
    ...
    const mockIncidents: TrafficIncident[] = [
      {
        ...
        description:
          "Right lane closed on I-45 South near Almeda Road. Expect delays.",
        severity: "medium",
        latitude: 29.7372,
        longitude: -95.3429,
      ...
      },
      {
        ...
        description:
          "Heavy congestion on US-59/I-69 Southwest Freeway near Galleria area.",
        severity: "high",
        latitude: 29.7372,
        longitude: -95.4618,
      ...
      },
      {
        ...
        description:
          "Multi-vehicle accident on I-610 West Loop has been cleared. Traffic normalizing.",
        severity: "low",
        latitude: 29.7849,
        longitude: -95.4618,
  ...
  try {
    ...
    for (const incident of incidents) {
      ...
      if (existing) {
        ...
        // Insert new incident
        const { error } = await supabase.from("houston_traffic").insert({
          title: incident.title,
          description: incident.description,
          severity: incident.severity,
  ...
  try {
    ...
    for (const alert of alerts) {
      ...
      if (existing) {
        ...
        // Insert new alert
        const { error } = await supabase.from("houston_metro_updates").insert({
...
serve(async (req) => {
  ...
  try {
    ...
    console.error("Error in houston-traffic function:", error);
    return new Response(
      JSON.stringify({
