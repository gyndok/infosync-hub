    ...
    if (action === "list") {
      ...
      if (apiKey) {
        ...
        if (urlParams.toString()) {
          fullTestUrl +=
            (fullTestUrl.includes("?") ? "&" : "?") + urlParams.toString();
        }
      }
      ...
      const healthSummary = secretsStatus.map((secret) => {
        const recentChecks =
          recentHealth?.filter((h) => h.service_name === secret.service_name) ||
          [];
        const isHealthy =
          recentChecks.length > 0 ? recentChecks[0].is_healthy : null;
      ...
      return new Response(
        JSON.stringify({
          ...
          services_with_secrets: secretsStatus.filter((s) => s.has_secret)
            .length,
