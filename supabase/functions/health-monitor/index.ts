  ...
  try {
    ...
    if (action === 'check-all') {
      ...
      const { data: services, error: servicesError } = await supabase
        .from('api_services')
        .select('*')
        .eq('is_enabled', true) as { data: ApiServiceConfig[] | null, error: any };
      ...
      const healthSummary = services.map(serviceConfig => {
        const serviceLogs = recentLogs?.filter(log => log.service_name === serviceConfig.service_name) || [];
        const totalRequests = serviceLogs.length;
        const healthyRequests = serviceLogs.filter(log => log.is_healthy).length;
        const avgResponseTime = serviceLogs.length > 0 
          ? Math.round(serviceLogs.reduce((sum, log) => sum + (log.response_time_ms || 0), 0) / serviceLogs.length)
          : 0;

        return {
          service_name: serviceConfig.service_name,
          is_enabled: serviceConfig.is_enabled,
          uptime_percentage: totalRequests > 0 ? Math.round((healthyRequests / totalRequests) * 100) : 100,
          avg_response_time_ms: avgResponseTime,
          total_requests_24h: totalRequests,
          last_check: serviceLogs[0]?.checked_at || null,
          current_status: healthChecks.find(hc => hc.service_name === serviceConfig.service_name)?.is_healthy || false
        };
      });
...
async function checkServiceHealth(supabase: any, serviceConfig: ApiServiceConfig) {
